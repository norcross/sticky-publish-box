...
function SES_SideStickyLoader( SES_ScreenSize, stickyID ) {
    ...
    if ( ! jQuery( 'div#side-sortables' ).length ) {
        return;
    }
    ...
    if ( SES_ScreenSize > stickyVars.breakpoint ) {
        SES_SideSticky( 'div#side-sortables', stickyID );
    }
	...
	if ( jQuery( stickyID ).hasClass( 'closed' ) ) {
		...
		// clear out any stuff
		SES_SideStickyRemoval( 'div#side-sortables', stickyID );
...
function SES_SideSticky( stickyWrap, stickyID ) {
    ...
    if( SES_IsScrolledTo( stickyAttach ) && ! jQuery( stickyID ).hasClass( 'closed' ) ) {
        ...
        jQuery( stickyWrap ).find( 'div.postbox' ).not( stickyAttach ).css( 'opacity', stickyVars.opacity );
    }
...
function SES_SideStickyRemoval( stickyWrap, stickyID ) {
    ...
    jQuery( stickyWrap ).find( 'div.postbox' ).not( 'div.sticky-side-wrapper' ).css( 'opacity', '' );
}
...
//*****************************************************************************************************
	$( stickyVars.stickyid ).on( 'click', 'div.handlediv', function() {
    	if ( ! $( stickyVars.stickyid ).hasClass( 'closed' ) ) {
    		SES_SideStickyLoader( SES_ScreenSize, stickyVars.stickyid );
    	} else {
    		SES_SideStickyRemoval( 'div#side-sortables', stickyVars.stickyid );
    	}
	});
